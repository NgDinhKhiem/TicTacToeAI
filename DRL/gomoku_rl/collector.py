...
def make_transition(
    ...
    # Select log prob key - ProbabilisticActor may use either sample_log_prob or action_log_prob
    log_prob_key = None
    if "action_log_prob" in tensordict_t_minus_1.keys():
        log_prob_key = "action_log_prob"
    elif "sample_log_prob" in tensordict_t_minus_1.keys():
    ...
    # Add log prob with the correct key name for PPO
    if log_prob_key:
        transition.set("action_log_prob", tensordict_t_minus_1.get(log_prob_key))
    transition.set(
        "next",
